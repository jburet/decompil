...
public class OpCodeInterpreter {
	...
	// Le but va être de construire une premiere structure (sous forme de block
	// de code).
	public MethodInstruction constructTree(MethodInfo methodInfo) {
		...
		for (short i = 0; i < code.length; i++) {
			...
			switch (opc) {
			// Cas d'un if avec une condition par rapport à 0;
			case ifeq:
			case ifne:
			case iflt:
			case ifle:
			case ifgt:
			case ifge:
				// Comparaison d'un int par rapport à 0
				// Le int à comparer se récupére dans la pile
			...
			case getstatic:
				// On récupère le field du constant pool
				// FIXME On génère de ObjectReference ou un ArrayReference selon le type
				tmpIndex = getIndex(code[++i], code[++i]);
				// FIXME on opérand on récupère la class du constantpool
			...
			case invokestatic:
				...
				// On récupère la référence ce la méthode à appeler
				...
				if (returnType.getDescriptorType() != DescriptorType.VOID) {
					// S'il y a un retour alors c'est un operand, le résultat
					// est stocke dans la stack (c'est une
					// autre
					// instruction qui contient le résultat de la méthode)
					operandStack
			...
			case anewarray:
				...
				operandStack.push(new ConstantArrayReference(ArrayType.T_REF, ClassFileUtils.decodeUTF(methodInfo
						.getReferentClassFile(), constantClass.getNameConstantIndex()), operandStack.pop()));
				break;

			case arraylength:
				// Appel à l'instruction java length sur un tableau
				operandStack.pop();
				// FIXME On push une primitive de type entier récupérer du
				// champs d'un object
			...
			case aastore:
				currentInstruction.addInstruction(currentPosition, new AssignationArrayInstruction(currentPosition,
						operandStack.pop(), operandStack.pop(), (ArrayReference) operandStack.pop()));
				break;

			case iaload:
			case baload:
				operandStack.push(new ArrayAccessInstruction(operandStack.pop(), (ArrayReference) operandStack.pop()));
				break;
			...
			case getfield:
				constantField = (ConstantField) constants[getIndex(code[++i], code[++i]) - 1];
				constantNameType = (ConstantNameType) constants[constantField.getNameTypeIndex() - 1];
				// FIXME Définition du type (disponible dans constantNameType)
			...
			// Switch
			case tableswitch:
				// Des bytes de padding peuvent être present pour que le début
	...
	private Constant getConstantFromConstantPool(int constantIndex, ClassFile cf) {
		...
		case ConstantType.STRING:
			...
			break;
		// TODO Décodage de tous les type numérique
	...
	private String getVariableNameFromRef(int tmpIndex, Map<Integer, Variable> localVariable, Operand operand) {
		...
		if (var == null) {
			if (operand instanceof ConstantArrayReference) {
				...
				VariableArrayReference vArrayRef = new VariableArrayReference((ConstantArrayReference) operand, "local"
						+ tmpIndex);
	 ...
	 * @return
	 * 
	 *         Construit l'index à partir des 2 octets suivant une
